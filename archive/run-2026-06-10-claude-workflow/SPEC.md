...
All code lives under `generated-src/`. Only add files that are described below, don't add any extra
files:
...
Standard CG on the flattened interior system. Define `A(p)` as the application of the discrete
Laplacian to a vector `p` reshaped to `(nx, ny)`. Convergence on `‖r‖₂ < tol`.

Assemble the sparse `(nx*ny) × (nx*ny)` Laplacian matrix and solve with
`scipy.sparse.linalg.spsolve`.

For a Neumann edge, zero-flux means ghost cell value equals the adjacent interior value. Apply
before each residual evaluation.
